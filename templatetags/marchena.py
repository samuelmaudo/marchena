...
## {% get_author username[ as variable_name] %} ################################
...
## {% get_authors *usernames as variable_name %} ###############################
...
## {% get_blog blog_slug[ as variable_name] %} #################################


class GetBlogTag(AssignTag):

    assign_var = False

    def process(self, blog_slug):
        return Blog.cache.get(slug=blog_slug)

register.tag('get_blog', GetBlogTag.as_tag())


## {% get_blog_category category_slug[ as variable_name] %} ####################


class GetBlogCategoryTag(SingleObjectMixin, AssignTag):
...
register.tag('get_blog_category', GetBlogCategoryTag.as_tag())


## {% get_blog_categories *category_slugs as variable_name %} ##################


class GetBlogCategoriesTag(MultipleObjectMixin, AssignTag):
...
register.tag('get_blog_categories', GetBlogCategoriesTag.as_tag())


## {% get_blogs *blog_slugs as variable_name %} ################################


class GetBlogsTag(AssignTag):

    def process(self, *blog_slugs):
        if blog_slugs:
            return Blog.cache.get_many(slug=blog_slugs)
        else:
            return Blog.cache.all()

register.tag('get_blogs', GetBlogsTag.as_tag())


## {% get_next_post[ in_same_blog][ as variable_name] %} #######################
...
class GetNextPostTag(AssignTag):
    ...
    def process(self, in_same_blog=True):
...
## {% get_previous_post[ in_same_blog][ as variable_name] %} ###################
...
class LastPublicationDateTag(MultipleObjectMixin, SingleTag):
...
register.tag('last_publication_date', LastPublicationDateTag.as_tag())


## {% last_updated[ format] %} #################################################


class LastUpdatedTag(LastPublicationDateTag):

    def get_date(self, queryset):
        queryset = queryset.order_by('-last_modified')
        return queryset.values_list('last_modified', flat=True).first()

register.tag('last_updated', LastUpdatedTag.as_tag())
